...
| DEMO-3 | Preflight demonstrates matching return data and baseline-verified fallback | Live production browser pass | On 2026-08-28 the compatible sample pinned block `0x3755a9a`, displayed matching original/attributed return data, and selected attributed calldata; the strict sample selected the exact 36-byte original only after its baseline succeeded and the attributed call reverted | Equal return data remains point-in-time evidence, not proof of equal state effects or future execution |
| UX-1 | Core states, accessibility, and responsive layout are usable | Live production browser pass | 10 demo tests cover strict-sample identity, full sample recovery, validation, comparison presentation, SSR, and the production handler; gstack v1.71 verified the deployed Worker at 1280×720 and 390×844, measured no mobile horizontal overflow, and found no console errors | A formal third-party WCAG audit is not claimed |
