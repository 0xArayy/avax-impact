...
Assessment date: **2026-08-29**. “Pass locally” means verified in this checkout.
“Recorded live pass” means the deployment manifest records a successful live check and
the repository provides the command to repeat it. “Production browser pass” records a
manual check of the public Cloudflare deployment identified below. External adoption and
security review require evidence outside this repository and are not inferred from code.

Production browser acceptance was repeated against
<https://avax-impact.0xarayy.workers.dev> at Cloudflare Version ID
`8fdbfd82-b6df-4435-85d7-b7f025991f34` on 2026-08-29.
...
| CLI-1 | Expose schema 1 encode, decode, resolve, and preflight workflows with explicit historical commands | Public GitHub release pass | CLI requires registry context for `encode`/`preflight`; `encode-legacy` and `resolve-legacy` cannot be selected accidentally; both export paths ship in `v0.1.1` | Publish to npm only after registry credentials are configured |
| QA-1 | Core SDK and contract suites pass with SDK coverage floors | Pass locally | `npm run test:sdk`: 48/48 with ≥85% lines, ≥70% branches, ≥95% functions; `npm run test:contracts`: 20/20 on 2026-08-29 | Add a stable Solidity coverage threshold when the pinned Foundry runner can produce it consistently; `npm run check` owns repository-wide QA |
...
| DEMO-2 | Inspect defaults to confirmed schema 1 Fuji evidence and preserves provenance | Production browser pass | Live Inspect decoded transaction `0x2e82…3530` as schema 1, showed chain `43113`, embedded registry `0x9695…4653`, and the registered, valid `avax-impact` record | Maintain the confirmed-transaction canary and keep historical schema 0 evidence secondary |
| DEMO-3 | Preflight demonstrates schema 1 matching return data and baseline-verified fallback | Production browser pass | The live compatible sample retained attributed calldata after matching return data; the strict sample selected untouched original calldata after the attributed call reverted at the same pinned block | Repeat after SDK, RPC, contract, or deployment changes; equal return data remains point-in-time evidence |
| UX-1 | Core states, accessibility, and responsive layout are usable | Production browser pass | 12 demo tests pass; live desktop and 390×844 checks had no console errors or horizontal overflow, and exposed the safety disclaimer without wallet or signing surfaces | A formal third-party WCAG audit is not claimed |
...
| RELEASE-1 | SDK artifact is installable by a clean consumer | Public GitHub release pass | The `v0.1.1` workflow passed the full gate and published `avax-impact-sdk-0.1.1.tgz`; its downloaded SHA-256 is `194be1b0469271060ca6ee02dae2495ab516161d3736c07c4718972a864d8af8` | Publish to npm only after registry credentials are configured |
